...
Once the data has been prepared for Stan, the model can be run. This example has a sub-optimal number of iterations and chains. We recommend using the defaults, 4 chains, 1000 iterations for both warmup and sampling, and assessing convergence. Note that this step usually takes a long time (e.g., 6-12 hours, or even days depending on the species and/or model).
...
Convergence should be checked for all models, and users should be concerned if Rhat >1.01 or ess_bulk <400. Convergence can also be checked visually, all chains should have similar distributions. If convergence fails, re-running the model with a longer burn-in and/or more posterior iterations or greater thinning rates may improve convergence. The seriousness of these convergence failures is something the user must interpret for themselves. In some cases some parameters of the model may not be separately estimable, but if there is no
...
```
<img src= "figures/mcmc_density-1.png" />

### Posterior Predictive Checks

Posterior predictive checks are best done in R, after fitting the model. It is certainly possible to modify the Stan model to include posterior predictions of the individual BBS counts. However, using Stan to generate these predicted counts will almost double the size of the saved model object. 
To generate the posterior distributions of predicted counts, you will need to extract the posterior samples of the parameter `E` in the bbsBayes2 models. Also, if you have used the negative binomial error distribution, then you will also need to extract the posterior samples for the parameter `phi` (the inverse negative binomial dispersion parameter).
...
overplot <- bayesplot::ppc_ecdf_overlay(y = heta_usgs_gamye$raw_data$count,
                                    yrep = y_rep)
overplot <- overplot +
  coord_cartesian(xlim = range(heta_usgs_gamye$raw_data$count))
...
#make a multiplot, without y axis labels

gt<-patchwork::patchworkGrob(pp)
#get the plot as a grob so we can...
gridExtra::grid.arrange(gt,left="Annual index of abundance /n (mean count)")
#add one y axis!

#or we could simply
patchwork::wrap_plots(p, ncol = 3) #with multiple y axis titles
```
<img src = "figures/plot_indices-1.png" />
...
<img src="figures/HETA_cont-1.png"/>

```{r}
tp[[2]]
```

<img src="man/figures/HETA_103-1.png"/> etc.
...
<img src="man/figures/HETA_trend-1.png"/>
...
<img src="figures/HETA_slope-1.png"/>
...
<img src="man/figures/GAM_ind-1.png"/>
...
<img src="man/figures/gamye_plot-1.png"/>
...
first_prep<-prepare_model(data_gam_first,model="first_diff")
...
first_ind <- generate_indices(gamye_mod,regions = c("continent"))
  
plot_indices(first_ind)   
```

<img src="man/figures/first_ind-1.png"/>
