...
model {
  ...
  sdyear ~ normal(0,0.3); // prior on sd of yeareffects - stratum specific, with a prior that suggests 99% of probability mass is for values < 0.77 and that
  // annual increases of ~50% and decreases of 40% are common, but 3-4 fold annual increases or decreases are unlikely
  ...
  sum(obs_raw) ~ normal(0,0.001*n_observers); // constraint

  ste_raw ~ std_normal();//site effects
  sum(ste_raw) ~ normal(0,0.001*n_sites); //constraint
