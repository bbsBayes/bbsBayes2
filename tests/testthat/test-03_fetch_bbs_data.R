...
test_that("have_bbs_data() / remove_cache()", {
  skip_on_ci()
  expect_message(h <- have_bbs_data(), "Expected BBS state data 2026")
  expect_message(remove_cache(level = "state", release = 2026))
  expect_message(remove_cache(level = "state", release = 2026),
...
test_that("get_XXXX()", {
  ...
  connection <- sbtools::item_get(sb_id = get_sb_id(release = 2020))

  expect_message(b <- get_birds("state", release = 2020, quiet = FALSE, connection, TRUE))
  expect_s3_class(b, "tbl")
  expect_message(b <- get_birds("stop", release = 2020, quiet = FALSE, connection, TRUE))
  expect_s3_class(b, "tbl")

  expect_message(r <- get_routes(2020, quiet = FALSE, connection, TRUE))
  expect_s3_class(r, "tbl")

  expect_message(w <- get_weather(2020, quiet = FALSE, connection, TRUE))
  expect_s3_class(w, "tbl")


  connection <- sbtools::item_get(sb_id = get_sb_id(release = 2026))

  expect_message(b <- get_birds("state", release = 2026, quiet = FALSE, connection, TRUE))
  expect_s3_class(b, "tbl")
  expect_message(b <- get_birds("stop", release = 2026, quiet = FALSE, connection, TRUE))
  expect_s3_class(b, "tbl")

  expect_message(r <- get_routes(2026, quiet = FALSE, connection, TRUE))
  expect_s3_class(r, "tbl")

  expect_message(w <- get_weather(2026, quiet = FALSE, connection, TRUE))
  expect_s3_class(w, "tbl")
...
test_that("fetch_bbs_data()", {
  ...
  release <- c(2020, 2022, 2023, 2024, 2025, 2026)[6]    # Add [2] to test only 2022
